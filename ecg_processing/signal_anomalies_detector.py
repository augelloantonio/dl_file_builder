    ...
    ** args:
    ...
    for i in range(0, len(nni)):
        ...
        if i < len(nni)-2:
            ...
            """ C1 """
            if (nni[i+1] < 600) and (nni[i+2] < nni[i]): #c1
                print(i)
                while is_ventriculat_rythm:  
                    # print("HERE AFIB")
                    # print(pulse)
                    ...
                    if (nni[x] < 800 and nni[x+1] < 800 and nni[x+2]<800) or ((nni[x] + nni[x+1] + nni[x+2])< 1800): #c2
                        ...
                        ecgAnomaliesList[-1] = "vt/vf" # CATEGORY 3
                        ecgANList[1] = "vt/vf" # CATEGORY 3
                        beat_list_peaks.append(rpeaks[x+2])
                        x = x+1
                        ...
                        if pulse < 4:
                            ...
                            beat_list_peaks.append(rpeaks[x % 3])
                            x = x-pulse
                            ecgANTotal.append("N") 
                        else:
                            for x in range (x, pulse):
...
def countAnomalies3(beatClassificationList, peaks):
    count_n = 0
    count_pvc = 0
    count_escape_beats = 0
    count_fib = 0
